...
- [ ] **Pager**: mute the group on the phone; the log shows `pager reachable` with one operator at daemon start; make this session's agent ask a question → no sound from the topic (the post is there, silent, with its buttons), one sound from the bot's private chat with `❓ <agent> is waiting for you`, the options and a link that opens the topic at the post; answer in the topic; the log has `screen posted … notify=false paged=true` and `pager sent`
