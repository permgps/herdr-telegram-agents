...
func TestTranslate(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:  "400 message to edit not found",
			in:    lib(bot.ErrorBadRequest, "Bad Request: message to edit not found"),
			is:    []error{domain.ErrMessageGone, bot.ErrorBadRequest},
			notIs: []error{domain.ErrTopicGone, ErrMessageNotModified},
		},
		{
			name: "400 message to delete not found",
			in:   lib(bot.ErrorBadRequest, "Bad Request: message to delete not found"),
			is:   []error{domain.ErrMessageGone},
		},
		{
			name: "400 message to pin not found",
			in:   lib(bot.ErrorBadRequest, "Bad Request: message to pin not found"),
			is:   []error{domain.ErrMessageGone},
		},
		{
			name:    "400 message can't be edited stays APIError",
			in:      lib(bot.ErrorBadRequest, "Bad Request: message can't be edited"),
			notIs:   []error{domain.ErrMessageGone},
			apiCode: 400,
			descHas: "can't be edited",
		},
