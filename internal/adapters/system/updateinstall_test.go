...
func TestUpdateInstallerLocalBackupInstallVerifyRollback(t *testing.T) {
	...
	var commands []string
	i := &UpdateInstaller{StateDir: stateDir,
		...
		RunCommand: func(_ context.Context, _ string, binName string, args ...string) error {
			commands = append(commands, binName+" "+strings.Join(args, " "))
			if binName == "sh" {
	...
	if len(commands) != 2 || commands[0] != "git merge --ff-only new-commit" || commands[1] != "sh scripts/install.sh" {
