...
func TestActionDispatch(t *testing.T) {
	tests := []struct {
		...
		{id: "status", sup: fakeSupervisor{status: compose.DaemonStatus{Running: true, PID: 5, Since: time.Now()}}, wantCode: exitOK, wantOut: "daemon running (pid 5", wantSup: []string{"status"}},
		{id: "status", wantCode: exitOK, wantOut: "daemon not running", wantSup: []string{"status"}},
		...
		{id: "stop", sup: fakeSupervisor{err: domain.ErrUnsupportedPlatform}, wantCode: exitError, wantOut: "Windows milestone", wantSup: []string{"stop"}},
